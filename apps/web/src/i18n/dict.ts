...
const zh = {
  ...
  rules: {
    title: "作答规则",
    sub: "问卷自动作答的关键词库，移植自 2023 年初版，是本项目最有价值的资产。改动经心跳下发，≤15 秒生效。",
    kindAnswer: "命中即勾选",
    kindAnswerHint: "选项文本含任一关键词就勾上（OR）",
    kindManual: "挂起交人工",
    kindManualHint: "题干需含全部关键词才挂起（AND）——加词等于收紧判据",
    kindNegation: "命中即排除",
    kindNegationHint: "含任一标记的选项一律不勾，优先级高于「命中即勾选」",
    cat: {
      skincare: "护肤品类",
      haircare: "美发",
      makeup: "彩妆",
    ...
    noHits: "未命中",
    hitsTitle: "命中流水",
    hitsSub: (n: number) => `在已采集的真实题库里命中 ${n} 处，不必再上站点核对`,
    hitsEmpty: "在已采集的题库里一条都没命中 —— 可能站点措辞变了，也可能这条词本就是为别的问卷写的。",
    hitsMore: (n: number) => `另有 ${n} 条未列出`,
    suppressedBy: (kw: string) => `实际不会勾：选项里同时含反转标记「${kw}」`,
    enable: "启用",
    disable: "停用",
    disabled: "已停用",
    edit: "编辑",
    ...
    addNote: "备注（可选）",
    submit: "添加",
    countOf: (on: number, all: number) => `${on}/${all} 启用`,
    search: "搜关键词或分类",
    onlyDisabled: "只看已停用",
    onlyNoHits: "只看未命中",
    reset: "清除筛选",
    empty: "没有符合条件的规则",
    conflictHint: "这条词同时也是反转标记，永远不会生效",
  },
...
const ja: Dict = {
  ...
  nav: { console: "コンソール", records: "履歴", diagnostics: "診断", rules: "回答ルール", settings: "設定", back: "戻る" },
  ...
  rules: {
    title: "回答ルール",
    sub: "アンケート自動回答のキーワード辞書。2023 年の初版から移植した、本プロジェクト最大の資産です。変更はハートビート経由で 15 秒以内に反映されます。",
    kindAnswer: "一致したら選択",
    kindAnswerHint: "選択肢にいずれかの語が含まれれば選ぶ（OR）",
    kindManual: "手動選択に回す",
    kindManualHint: "設問文にすべての語が含まれる場合のみ保留（AND）——語を足すほど条件は厳しくなります",
    kindNegation: "一致したら除外",
    kindNegationHint: "いずれかの語を含む選択肢は選ばない。「一致したら選択」より優先",
    cat: {
      skincare: "スキンケア",
      haircare: "ヘアケア",
      makeup: "メイク",
    ...
    noHits: "一致なし",
    hitsTitle: "一致した実データ",
    hitsSub: (n: number) => `収集済みの実アンケートで ${n} 件に一致（サイトを再訪する必要はありません）`,
    hitsEmpty: "収集済みの設問集では一件も一致しません —— サイトの表現が変わったか、別のアンケート向けの語かもしれません。",
    hitsMore: (n: number) => `ほか ${n} 件は非表示`,
    suppressedBy: (kw: string) => `実際には選ばれません：選択肢に反転マーカー「${kw}」も含まれています`,
    enable: "有効化",
    disable: "無効化",
    disabled: "無効",
    edit: "編集",
    ...
    cancel: "キャンセル",
    remove: "削除",
    builtinNoDelete: "初期搭載の語は無効化のみ可能で、削除はできません",
    add: "ルールを追加",
    addKeyword: "キーワード",
    addCategory: "分類",
    addKind: "種別",
    addNote: "メモ（任意）",
    submit: "追加",
    countOf: (on: number, all: number) => `${on}/${all} 有効`,
    search: "キーワード・分類で検索",
    onlyDisabled: "無効のみ",
    onlyNoHits: "一致なしのみ",
    reset: "絞り込み解除",
    empty: "該当するルールはありません",
    conflictHint: "この語は反転マーカーにも登録されているため、決して有効になりません",
  },
...
const en: Dict = {
  ...
  nav: { console: "Console", records: "Records", diagnostics: "Diagnostics", rules: "Rules", settings: "Settings", back: "Back" },
  ...
  rules: {
    title: "Answer rules",
    sub: "Keyword library for auto-answering surveys, ported from the 2023 original — this project\u2019s most valuable asset. Changes ship on the next heartbeat (\u226415s).",
    kindAnswer: "Select on match",
    kindAnswerHint: "Tick an option if its text contains any keyword (OR)",
    kindManual: "Escalate to human",
    kindManualHint: "Escalate only if the prompt contains every keyword (AND) — adding a word tightens the test",
    kindNegation: "Exclude on match",
    kindNegationHint: "Never tick an option containing any of these; takes precedence over Select on match",
    cat: {
      skincare: "Skincare",
      haircare: "Hair care",
      makeup: "Makeup",
      body: "Body & sun care",
      oral: "Oral care",
      channel: "Where bought & sources",
      concern: "Skin type & concerns",
      effect: "Claims & finish",
      attitude: "Attitude & intent",
      frequency: "Frequency & amount",
      consent: "Entry consent",
      manual: "Manual-choice markers",
      negation: "Negation markers",
      custom: "Custom",
    },
    hits: (n: number) => `${n} hits`,
    noHits: "no hits",
    hitsTitle: "Matches in captured surveys",
    hitsSub: (n: number) => `Matched ${n} times across the captured real surveys — no need to revisit the site`,
    hitsEmpty: "No matches in the captured question bank — the site wording may have changed, or this word was written for a different survey.",
    hitsMore: (n: number) => `${n} more not listed`,
    suppressedBy: (kw: string) => `Never actually ticked: the option also contains the negation marker \u201c${kw}\u201d`,
    enable: "Enable",
    disable: "Disable",
    disabled: "Disabled",
    edit: "Edit",
    ...
    cancel: "Cancel",
    remove: "Delete",
    builtinNoDelete: "Built-in words can be disabled but not deleted",
    add: "Add rule",
    addKeyword: "Keyword",
    ...
    submit: "Add",
    countOf: (on: number, all: number) => `${on}/${all} enabled`,
    search: "Search keyword or category",
    onlyDisabled: "Disabled only",
    onlyNoHits: "No hits only",
    reset: "Clear filters",
    empty: "No rules match",
    conflictHint: "This word is also a negation marker, so it can never take effect",
  },
