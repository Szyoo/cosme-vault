...
- **首页有「奖品概览」**（`present-overview.tsx`，2026-08-27 用户要求）：按类型与状态
  的分布放在页面靠上处——这些数字原先只在列表的筛选栏里，而列表在一百多条奖品之后的
  页尾，等于要滚到底才看得见。chip **可点**，筛的就是下面那份列表并把视线带过去；
  做成死数字的话，上面一排点不动、下面一排一模一样的能点，反而更迷惑。
  - 因此筛选状态提到了 `present-filter.tsx` 的 context（概览与列表在页面上隔得远，
    塞不进同一个组件）。⚠️ **记录页也用 `PresentList`，同样要包 Provider**，
    否则 useContext 取不到直接抛。
  - 状态口径是「任一账号处于该状态」，故**各状态之和可能大于奖品总数**——
    界面上要写明，不解释会被当成算错。
