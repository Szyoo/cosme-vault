...
- **每次扫描完终端有一条汇总行**（`applyReport` 的 scan 分支写 `runner_logs`）：
  「扫描完成：站上 N 个奖品 · 新奖品 X 个 · 新建待投递记录 Y 条 · 已派发投递 Z 个」，
  有来源没认出版式则追加 ⚠️ 并记为 warn。
  ⚠️ 汇总**必须写在控制面**：逐来源流水由 runner 打，但「哪些是新的」只有控制面
  知道（它才有旧数据）。而且要写在派发之后，否则带不上派发数。
  没有这行的时候，一次扫描打十几行来源流水却没有结论，「今天没新奖品」和
  「某来源悄悄失效」看起来一模一样。
- **终端的「清空」只清屏、不删数据**：记一个水位线（当前最新那条日志 id），
  只隐藏 id ≤ 水位线的行，新日志照常出现，库里一条不动。水位线存 localStorage
  （否则按一下 F5 就全回来了，等于没清）；⚠️ **只能在 effect 里读 localStorage**，
  渲染期读会水合不一致。清完显示「已隐藏 N 条（仍保留在日志里）· 显示全部」——
  不说清楚没人敢按，也得给回退入口。
- ⚠️ **`TermLog` 的文案改为自己 `useT()`，不走 props**：`hidden(n)` 是函数，
  函数不能从服务端组件传给客户端组件（Next 会当成 Server Action 报错）。
  与 present-list / account-matrix 的做法一致。
- **终端按任务分段**：日志行带 `jobId`，换任务就画一条双线（`term-sep`）。
  此前只能靠时间戳猜哪几行属于哪次任务。用 CSS 双线而非一串「─」字符——
  固定长度的字符串在 375px 上会折成两截。复制出去的文本也带 `═` 分隔。
...
- **奖品列表是「一奖品一行」，账号状态收在行内**（2026-08-25 修，用户指出）：
  ...
  行内每个账号一枚状态 pill（多账号时带账号短名）；状态筛选按
  「**任一账号**处于该状态」计数与过滤，否则「A 已投 B 待投」的奖品会被漏掉。
- **首页有「奖品概览」**（`present-overview.tsx`，2026-08-27 用户要求）：回答
  **「现在还有多少能抽」**。chip 可点，筛的就是下面那份列表并把视线带过去；
  做成死数字的话，上面一排点不动、下面一排一模一样的能点，反而更迷惑。
  - ⚠️ **这里的「状态」是奖品自己的，与账号无关**（用户明确指出，我第一版搞混了）：
    投没投是**账号**的事（上面的「各账号进度」已经在答），「还在不在募集」是**奖品**
    的事。混成一个维度就等于两个问题都没答清。
  - `PresentItem.life`（`lifeOf`，服务端算好）= active / expired / gone，两路证据：
    任一账号跑到过 gone/expired（runner 在站点上亲眼看到的结论，下架对所有账号成立），
    **或**期间已过（`isPeriodExpired`）。⚠️ 第二路不能省——它兜住「还没有任何账号
    跑过」的那些，实测多出 **32** 个（账号维度 59 vs 奖品维度 91），少了这一路
    它们会一直以「募集中」的面目挂在概览里。
  - **类型分布只统计募集中的**：把已下架的算进去，「PR 合作 110」看着还有 110 个能投，
    其实早没了。点类型 chip 会连带把 life 设为 active，与看到的数字一致（所见即所得）。
  - 筛选状态在 `present-filter.tsx` 的 context（概览与列表在页面上隔得远，塞不进同一个
    组件）。⚠️ **记录页也用 `PresentList`，同样要包 Provider**，否则 useContext 取不到直接抛。
...
## 未知模式的兜底阶梯（2026-08-24，用户要求）

`unknownPattern` 只该用于**真的没见过的版式**。此前 draw 只有「模式认领 / 未知模式」
两档，于是登录墙、已结束奖品页这些**认得出来**的页面全被报成未知模式
（127 个诊断包全是登录页）。现在的顺序（`cosme/page-kind.ts` + `draw.ts`）：

1. **先分类页面**（`classifyPage`）：`loginWall` / `ended` / `notFound` 各有确定结论——
   分别记 `failed`（附「去激活登录」指引）/ `skipped` / `skipped`，绝不进未知模式。
   入口处与流程中途落点都要过这一关。
2. **重试几次**：慢跳转/慢加载属瞬时问题，等一拍重新分类往往就好了（最多 3 轮）。
3. **全量重认模式**：落点换了地盘就交棒继续（跨模式接力，不再限一次）。
4. 以上都不成立才是真未知 → 安全中止 + 现场包。

⚠️ 奖品过期是**正常边界**，尤其第二个账号晚跑时常遇到——必须走 `ended` 分支记 skipped，
不是错误。

## 个人资料与站点选项的匹配（2026-08-25 事故）

第二个账号 81 个奖品全部失败，根因是**职业下拉没填上**：设置页填的是「自営業」，
站点选项是「自営業・自由業」——精确匹配 + 斜杠/中点互换都不中，下拉留空，
而它是必填，于是每次送信都被弹回。三处修正：

1. 职业匹配加**包含匹配**兜底（取最短候选，避免命中「その他（自営業を除く）」这类）；
   完全匹配不上时记  到日志，不再静默跳过。
2.  补上 ****：此前只看 radio/checkbox，下拉留空导致的
   弹回会落到「找不到未作答的题」→ ，用户连补救入口都没有。
3.  对  用 （ 对下拉无效）——
   否则用户从选择页补的下拉答案回填不上。

⚠️ 教训：**账号之间的差异首先怀疑个人资料**。同一奖品主号成功、副号失败，
日志里「个人资料 prof_001_name/age」少了  就是唯一线索。

## 异常的统一模型（2026-08-24，用户设计）

不再区分「未知页面」与「流程错误」在报出时的形态——**统一为一种「异常」**，
带可视证据与指纹，由出现次数决定要不要惊动人：

- **指纹去重**（`fingerprintOf` = 规范化 URL + 元素选择器集合的哈希）：
  同一种异常在 `anomalies` 表只有一行，累计 `seen_count`。127 个奖品撞同一登录墙
  → 诊断页 1 条「出现 127 次 · 影响 127 个奖品」，不是 127 份重复现场包。
  URL 规范化必须抹掉每次都变的参数（a_key/token/数字 id），否则去重白做。
- **可视证据是硬要求**：截图（JPEG data URI，视口尺寸质量 55）**内嵌回传**——
  `artifacts` 里那几个是 Mac mini 的本地文件路径，控制面在 VPS 上读不到，
  此前「看截图」完全落空。截图失败/超限则降级存 HTML 快照，诊断页用
  **sandbox iframe** 还原成看得见的页面（注入 `<base>` 让相对资源仍能加载）。
- **首次自动重试，重现才交人工**：某指纹第一次出现 → 奖品放回 `pending`
  下轮自动重试（多为瞬时问题，不烦人）；同指纹再次出现 → 判定可复现 →
  记 `unknownPattern` 交人工。
- 诊断页「标记已处理并重排」= 异常标记 resolved + 受影响奖品放回 pending
  （只改状态，**不派发投递**）。

## 扫描与账号的关系（2026-08-24 修正）

**扫描与账号无关**：同一站点的同一批奖品，一个账号扫过，`presents` 就是全局的；
`applyReport` 因此为**所有启用且有凭证的账号**建立 `account_presents` 记录，
不再只给执行扫描的那个账号（否则新账号必须自己再扫一遍才有记录，白跑且无意义——
实测后果是第二个账号 59 个奖品「未建记录」，界面上凭空缺一块）。
**投递仍严格按账号独立**——`account_presents` 是各账号的去重唯一防线。

⚠️ **「跑一轮」只入队一个 scan**（2026-08-27 修，用户指出扫了两遍）：此前 `startRun`
对每个启用账号各入队一个 scan，两个账号就把整站扫两遍——纯属白跑，还平白多一倍
站点访问（合规上更不该）。扫描仍需**某个**账号的登录态（brandfanclub 未登录看不到），
所以是「挑一个账号去扫」而非「为这个账号扫」，挑选优先 `sessionOkAt` 非空的
（拿没登录过的账号去扫必然撞登录墙）。
配套：scan 载荷带 `dispatchFor: string[]`（扫完给哪些账号派投递），`applyReport`
按它逐个派发。**这条不能靠 `accountId` 推**——扫描账号只有一个，那样另一个账号
在「跑一轮」里就永远拿不到投递任务。

## 界面上的状态归并与配色（2026-08-27，用户定的）

- **`alreadyEntered` 界面上并入「已投递」**（`labels.ts` 的 `mergeStatus`）：
  两者对用户的意义相同（已经参加了、不用再管），不必让人分辨。
  **库里仍分开存**——它是「这次什么都没提交、站点显示早已应募」，是任务中断后
  能安全自动重跑的依据。凡是按状态分组 / 筛选 / 计数的地方都要先过 `mergeStatus`，
  否则会出现两个都叫「已投递」的分组（首页 chip、列表筛选栏、记录页统计、
  404 改判下拉都踩得到）。
- **未知模式用明黄 `--unknown`，已下架保持酱黄 `--warn`**：进度条上同色段挨着分不出来。
- ⚠️ **`.pill-btn` 不能写 `font: inherit`**：那会把设计包 `.pill` 的 11px 覆盖成正文字号，
  chip 立刻比上面的 `btn-small`(13px) 还大（踩过）。只继承字体族，字号显式钉 11px。

## 触发的粒度（2026-08-24）

- **全局**（Runner 卡）：仅检测 / 仅抽取 / 跑一轮，作用于所有启用账号。
- **单账号**（账号矩阵每张卡）：同样三个按钮，只跑该账号（`POST /api/runs`
  带 `accountId`）。用户要求——两个账号进度不同时需要分别推进。
- 含投递的操作一律走**明示确认条**（说清后果 + 确认/取消，不设时限）。
  「再点一次」那种隐式二次确认已废除：文案突变 + 5 秒隐形时限，实测把用户卡死。

## 账号进度下钻（2026-08-27，用户要求）

账号矩阵的状态 chip 与色条段都可点，弹 modal 列出**该账号 × 该状态**的奖品与操作
（`account-matrix.tsx` 的 `DrillModal`）。要点：
...
- **职业是枚举，必须做成下拉**（`packages/core/profile-options.ts` 的 `JOB_OPTIONS`，
  出处是 v3/v4 两代项目里各存一份、完全一致的枚举）。
  ⚠️ 事故（2026-08-25）：用户手填「自営業」，站点选项是「自営業・自由業」，
  精确匹配不中 → **必填**下拉留空 → 送信被弹回，该账号 **81 个奖品全数失败**，
  另有 13 个挂成 needsChoice。设置页改下拉后值天然对得上；
  代码里仍保留包含匹配兜底（取最短候选），用于兜住历史数据与站点措辞微调。
- **姓名 / 年龄是自由文本输入**（`prof_001_name`、`prof_*age*`），无枚举对不上的风险。
- **未选中的 `<select>` 会升级成 needsChoice 推到网页**（`collectUnanswered`）：
  此前只看 radio/checkbox，于是日志只写「送信被退回，且找不到未作答的题」，
  用户连补救入口都没有。将来站点加别的必填下拉，也是弹给人选而不是静默失败。

## 状态必须单义（2026-08-26，用户指出）

原来的 `skipped`「已跳过」把两件事混在一起，界面上既说不清是奖品过期还是出了问题，
也分不清和「还没抽」的区别。已拆成两个：

- `expired`「已结束募集」—— 期间过了。**正常边界，不是错误**，尤其第二个账号晚跑时常遇到。
- `gone`「已下架」—— 页面 404，站点把这个奖品撤了。

同时**结论必须带依据**：`DrawResult.reason` 记「正文含结束文案：受付は終了」这类原话，
`applyReport` 优先把它写进 `error`。此前 expired/gone 的 `error` 全是空的，
于是界面上只有一个说不清缘由的「已跳过」（用户为此提过）。
**凡是非 drawn 的终态都该带一句 reason。**

历史 60 行经 `jobs.payload` → `runner_logs` 回溯当时的日志判定（迁移 `0011_split_skipped`）：
5 行 gone、55 行 expired，与日志统计逐条对上。

界面语义（2026-08-26 用户定的）：`expired` 显示为「已下架」（黄），`gone` 显示为
「404」（红）。**红=值得人看一眼**：404 要么是站点异常撤页，要么是我们存的链接有错。
账号进度条同套配色——已下架原用淡灰，和「待投递」的灰分不出区别（用户指出）。

**404 结论必须二次复核**（`draw.ts` 的 `reconfirmNotFound`）：等一拍重新加载再判，
仍 404 才定案。实测教训 `tu-8946`：c.w1.to 追踪链瞬时故障落到错误页，一次判定把
**还在募集中**的奖品（期间 8/5～9/1）记成了 404；事后匿名 curl 复核 HTTP 200、
「今すぐ応募」还在。已回填 pending。其余 4 条 gone 经匿名复核确认是真 404
（normal / produceMember 类过期后站点直接撤页面，404 是这两类的自然归宿）。

**`ENDED_TEXT` 判据已收紧为双条件（2026-08-26）：结束文案 且 应募入口已消失。**
单看文案会误伤——tieup 的 PR 页常并列多个活动，别的活动的「受付は終了」也在
innerText 里。实证依据（复核全部 55 条 expired，52 条期间硬证据 + 3 条页面文案证据，
零误判）：真结束的详情页应募入口一定没了，而 PR 页误扫场景本活动的「今すぐ応募」还在。
入口检测只看可点击元素（a/button/input 含图片按钮 alt），正文里「応募するには…」
这类说明句不算。入口还在就不定案，交给模式照常走流程——真结束了后续自然走不通，
会回到兜底阶梯（中途落点无入口 + 有終了文案 → 照样判 ended）。
