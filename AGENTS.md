...
## 作答规则页（2026-08-25，用户要求）

`/rules` 把问卷关键词库摊开成可增删改查的界面（此前只是 `keywords.ts` 里的一个数组）。

- **数据流**：出厂默认在 `@cosme/core` 的 `DEFAULT_RULES` → 首次访问播种进 `answer_rules` 表
  → 之后以表为准 → 经 `RunnerConfig.rules` 与节奏**同路下发** → runner 调
  `applyRuleOverrides()` 注入 core 的模块状态。改完 ≤15 秒生效，与节奏参数同一套机制。
  - ⚠️ **空列表一律回落到出厂词表**：控制面故障返回空数组时若照单全收，
    `answer` 空 = 一题都不作答、`negation` 空 = 该排除的照勾，两种都是静默错答。
  - ⚠️ `RunnerConfig` 拆出了 `PacingConfig`：设置页只提交节奏，共用一个 schema 的话
    节奏表单每次保存都得连规则一起回传，漏传就等于清空规则。
- **版面是「类型 → 分类 → 词条 chip」三层**：九十多条词平铺没法看，也无从判断某类是否缺词。
  分类**只影响展示、不影响匹配**（匹配一律是子串命中），内置分类是 slug（字典给三语标签），
  用户自建的分类原样显示。
- **三类语义不同，界面必须写明**：`answer` 命中任一即勾（OR）、`negation` 命中任一即排除
  （优先于 answer）、`manual` 题干需**全部**命中才挂起（**AND**——加词等于收紧判据，
  不写明会让人以为加词能多触发）。
- **「查」= 这条词到底命中了什么**，语料是 `survey_captures`（做题时顺手采的真实题库，
  现有 114 份问卷 3164 个选项）。**不需要额外埋点、不用再上站点**——这正是当初做问卷采集的目的。
  answer 类还会标出**被反转标记压掉**的命中（词表看着有效、实跑并不生效），这类最值得看。
- **出厂词只许停用、不许删**（`deleteRule` 拒绝 `builtin`）：每条都是 2023 年实跑调教出来的，
  删了没有复原依据，而停用同样达到不生效的目的。
- 实测基线（2026-08-25，114 份问卷）：101 条规则里 **51 条零命中**，40 处命中被反转标记压掉
  （全是 `@cosme以外のWEBサイト・アプリ` 命中 `＠ｃｏｓｍｅ` 被 `以外` 挡下——机制正常工作）。
  ⚠️ **零命中不等于失效**：题库只有最近这批（以 tieup 护肤类为主），口腔/美甲/防晒类
  根本没出现过。零命中是**线索**不是判决，界面文案照此措辞。
